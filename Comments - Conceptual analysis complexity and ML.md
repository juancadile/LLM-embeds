...
[TODO before sending — the numbers in this paragraph come from the per-pair values printed in the notebooks (74 pairs, the notebooks' own on-the-fly embeddings, L1 ratio formula). Only 10 of the 74 pairs are in the cached vocab, so they cannot be checked from the caches; a GPU re-extraction of all 74 pairs with the original recipe is running (results/REPORT.md, Phase 3). Rewrite this paragraph from that table, not from either of the current sets.]
